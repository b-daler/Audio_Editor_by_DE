...
class TrackSlider(tk.Canvas):
    ...
    def __init__(self, master, length, initial_position=0):
        super().__init__(
            ...
            bg='#D9D9D9', bd=0, highlightthickness=0, highlightbackground='#D9D9D9'
        )
        self.length = length
        self.slider = None
        ...
        if position == 0:
            ...
            if not self.progress_bar:
                self.progress_bar = self.create_rectangle(
                    ...
                    fill='black', outline='black', width=0, tags='progress'
                )
            else:
                self.coords(
    ...
    def hide_slider(self, event):
        ...
        if self.progress_bar:
            self.itemconfigure(self.progress_bar, fill='gray', outline='gray')
...
if __name__ == '__main__':
    ...
    slider = TrackSlider(root, track_length)
